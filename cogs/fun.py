...
class Fun(commands.Cog):
    ...
    async def eightball(
        ...
        question: discord.app_commands.Range[str, 0, 1000]
    ):
        answers = [
            'Yes.',
            'No.',
            ...
            'Possibly.',
            'Maybe.',
            'Better change the question.',
            'WTF?',
            '???',
            'This is an 8ball, not a reality changer.',
            'Nice question ngl.',
            'I don\'t understand.',
            'Well as you can see, the grass is green too. Wait- you\'ve never seen it!',
            'Another question like this and I\'ll blacklist you.',
            'SHUT UP!',
            'I DON\'T KNOW!'
        ]
...
'''
    ...
    async def percentage(
        self,
        interaction: discord.Interaction,
        target: discord.app_commands.Range[str, 0, 500],
        input: discord.app_commands.Range[str, 0, 500]
    ):
        embed = discord.Embed(
            color=2829617,
            description=f'{target} is **{random.randint(0, 100)}%** {input}.'
        )
    ...
    @discord.app_commands.describe(
        input='Type in what you want to rate.'
    )
    async def rate(
        self,
        interaction: discord.Interaction,
    ...
    @discord.app_commands.command(
        name='randomnumber',
        description='Send an random number.'
    )
    @discord.app_commands.describe(
        maximum='Type a maximum range.',
        minimum='Type a minimum range.'
    )
    async def randomnumber(
        ...
        number = round(random.randrange(minimum or 0, maximum))
